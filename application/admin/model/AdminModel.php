...
/*
 * 用于es_admin表操作的验证的一个类
 * @author liyuzhao
 *
 *以下一个函数功能
 * @access public
 * @abstract adminValide 主要功能：验证es_admin添加的信息。一个逻辑：主要对接收的admin信息进行验证
 * */

class AdminModel extends Model
{
    // 设置当前模型对应的完整数据表名称
    protected $table = 'es_admin';

    /*
     * 这是一个验证role添加的信息
     * @param array $data 接收的am_admin信息，为一个一维数组
     * @param string $val 默认为add，这个参数是表名是添加验证还是编辑验证,默认会对管理员进行存在验证
     * @return bool 验证正确返回true
     * @return string 失败则返回错误信息
     * */
    public function adminValidate($data, $val = 'add')
    {
        $validate = validate('AdminValidate');   //验证的规则，这个可以直接实例化到我admin模块下的validate目录下的Role类，里面我定义了规则
        $check_data = [
